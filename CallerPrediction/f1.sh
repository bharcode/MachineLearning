...
hadoop jar mahout-examples-0.7-job.jar org.apache.mahout.classifier.df.mapreduce.BuildForest -Dmapred.job.name=GRMS -Dmapred.job.queue.name=$MY_QUEUE -Dmapred.max.split.size=1000000 -d $PWD/mobile/train.csv -ds $PWD/mobile/mobile.info -sl 4 -p -t 100 -o ~/mob_pred_forest
if [ $? -ne 0 ]; then
...
hadoop jar mahout-examples-0.7-job.jar org.apache.mahout.classifier.df.mapreduce.TestForest -Dmapred.job.name=GRMS -Dmapred.job.queue.name=$MY_QUEUE -i $PWD/mobile/test.csv -ds $PWD/mobile/mobile.info -m ~/mob_pred_forest -a -mr -o $PWD/mobile_predictions
if [ $? -ne 0 ]; then
